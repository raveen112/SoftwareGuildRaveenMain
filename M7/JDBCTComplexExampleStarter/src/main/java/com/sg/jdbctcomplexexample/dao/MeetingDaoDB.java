...
public class MeetingDaoDB implements MeetingDao {
    ...
    // helper method #1; cleaner code since we set our queries in variables 
    public Room getRoomForMeeting(Meeting meeting) {
    ...
    // helper method #3; the above two together (each Meeting will have the Room and List of Employees added to it)
    ...
    public Meeting addMeeting(Meeting meeting) {    
        ...
        jdbc.update(INSERT_MEETING,
                meeting.getName(),
                // note in the "update" is how we handle passing the LocalDateTime field into the 
                //database: we have to wrap it in "Timestamp.valueOf" so that it looks correct when we write it to the database.
    ...
    // helper method#4; will handle creating the bridge table entries for the Meeting/Employee relationship.
    ...
    public void updateMeeting(Meeting meeting) {
       final String UPDATE_MEETING = "UPDATE meeting " + "SET name = ?, roomId = ? WHERE id = ?";
