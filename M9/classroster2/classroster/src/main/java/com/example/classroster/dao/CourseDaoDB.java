...
public class CourseDaoDB implements CourseDao{
    ...
    public Course getCourseById(int id) {
        try{
            final String SELECT_COURSE_BY_ID = "SELECT * FORM course WHERE id=?";
    ...
    private List<Student> getStudentForCourse(int id){
        final String SELECT_STUDENT_FOR_COURSE = "SELECT s. * FROM student s "+
                "JOIN course_student cs ON cs.studentId = s.id WHERE cs.courseId =?";
        return jdbc.query(SELECT_STUDENT_FOR_COURSE, new StudentMapper(), id);
    }
    @Override
    public List<Course> getAllCourses() {
        ...
        associateTeacherAndStudent(courses);
        return courses;
    }
    
    private void associateTeacherAndStudent(List<Course> courses){
    ...
    public List<Course> getCoursesForTeacher(Teacher teacher) {
        final String SELECT_COURSE_FOR_TEACHER = "SELECT * FROM course WHERE teacherId = ?";
        List<Course> courses = jdbc.query(SELECT_COURSE_FOR_TEACHER, new CourseMapper(),teacher.getId());
        associateTeacherAndStudent(courses);
        return courses;
    }
    ...
    public List<Course> getCoursesForStudent(Student student) {
        final String SELECT_COURSE_FOR_STUDENT = "SELECT c.* FROM course c JOIN "+ 
                "course_student cs ON cs.courseId = c.Id WHERE cs.studentId = ?";
        List<Course> courses = jdbc.query(SELECT_COURSE_FOR_STUDENT, new CourseMapper(), student.getId());
        associateTeacherAndStudent(courses);
        return courses;
    }
